...
use Drupal\kibo_sdk_proxy\Mozu\Api\Contracts\Core\UserAuthInfo;
use Drupal\kibo_sdk_proxy\Mozu\Api\Headers;
use Drupal\kibo_sdk_proxy\Mozu\Api\MozuConfig;
use Drupal\kibo_sdk_proxy\Mozu\Api\Urls\Platform\Adminuser\TenantAdminUserAuthTicketUrl;
use Drupal\kibo_sdk_proxy\Mozu\Api\Urls\Platform\Developer\DeveloperAdminUserAuthTicketUrl;
use Drupal\kibo_sdk_proxy\Mozu\Api\Utilities\HttpHelper;

class UserAuthenticator {
	
	public static function setActiveScope($authTicket, Scope $scope)
	{
		return static::refreshUserAuthTicket($authTicket, $scope->getId());
	}
	
	public static function ensureAuthTicket($authTicket)
	{
		$dateTimeNow = new DateTime();
		//$accessTokenExpiration = strtotime($authTicket->accessTokenExpiration);
		$accessTokenExpiratonDate =  new DateTime($authTicket->accessTokenExpiration);
	...
	public static function refreshUserAuthTicket($authTicket, $id = null)
	{
		try {
	...
	public static function logout($authTicket)
	{
		try {
			$authentication = AppAuthenticator::getInstance();
            //var_dump($authTicket);
	...
	private static function getResourceRefreshUrl($authTicket, $id = null)
	{
	...
	private static function getLogoutUrl($ticket)
	{
