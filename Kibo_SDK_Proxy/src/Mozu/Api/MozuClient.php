...
use Drupal\kibo_sdk_proxy\Mozu\Api\Security\AuthenticationScope;
use Drupal\kibo_sdk_proxy\Mozu\Api\Security\CustomerAuthenticator;
use Drupal\kibo_sdk_proxy\Mozu\Api\Security\UserAuthenticator;
use Drupal\kibo_sdk_proxy\Mozu\Api\Utilities\HttpHelper;
...
class MozuClient {
    ...
    public function __construct() {
        $this->logger = \Drupal::logger("MozuClient");
    }
	...
	private function setUserAuth() {
        ...
        $this->logger->info('Auth Token: ' . $authTicket->accessToken);
        $this->logger->info('Auth Token Exp: ' . $authTicket->accessTokenExpiration);
    ...
    private function getTenant() {
        ...
        $tenant = $tenantResource->getTenantAsync('28812')->wait();
    ...
    private function addRequestHeaders(RequestInterface $request) {
        ...
        if ($this->apiContext != null && $this->apiContext->getUserAuthTicket() != null) {
            $this->setUserAuth();
            $authToken = $this->apiContext->getUserAuthTicket();
            $this->logger->info("User Claim : ". $authToken->accessToken);
            $request = $request->withHeader(Headers::X_VOL_USER_CLAIMS, $authToken->accessToken);
        }
    ...
    private function validateResponse(ResponseInterface $response) {
        ...
        if ($statusCode == 404 && empty($correlationHdr))
            ...
            $this->logger->info(print_r($jsonResponse, true));
            if (!empty($jsonResponse)) {
                ...
                if (isset($jsonResponse->message))
                    ...
                    $apiError = new ApiException($response->getReasonPhrase().", inspect Drupal\kibo_sdk_proxy\Mozu\Api\ApiException->items property for more details");
                //$apiError = new ApiException($jsonResponse->message, $statusCode);
