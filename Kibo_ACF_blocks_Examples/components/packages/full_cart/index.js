...
import { updateInCart, removeCartItem, getCurrentCart } from 'acf/actions'
import FormSelect from 'acf/utils/FormSelect'
import ProductInCart from './ProductInCart'

const FullCart = ({cartState, productState, updateInCart, removeCartItem, getCurrentCart}) => {
  const {cartItems, count, totalPrice} = cartState
  const productsArray = Object.keys(cartItems)
    ...
    <div className="acf-full-cart">
      <div className="acf-full-cart__inner">
        <div className="acf-full-cart__main">
          ...
          <div><a href="#" onClick={handleRefresh}>Refresh</a></div>
          <div>Refreshes the cart from the server.(The Kibo Cart instance). <br/>
          Try removing an item, which dose not rerender the page, and then press the refresh to see the server update our component. <br/><br/>
          ** We are not really managing user creds here. Dumping the acf_store session stored will clear the current user token.
...
function mapDispatchToProps (dispatch) {
  return bindActionCreators(
    {updateInCart, removeCartItem, getCurrentCart},
    dispatch
  )
}
