...
use Drupal\acf_kibo\Mozu\Api\Security\AuthenticationScope;
use Drupal\acf_kibo\Mozu\Api\Security\CustomerAuthenticator;
use Drupal\acf_kibo\Mozu\Api\Security\UserAuthenticator;
use Drupal\acf_kibo\Mozu\Api\Utilities\HttpHelper;
...
class MozuClient {
	...
	private function buildClientAndRequest() {
        ...
        $this->client = new Client (['handler'=>$stack,'base_uri' => $this->baseAddress,  'verify' => false, 'proxy' => '127.0.0.1:8888',]);
        ...
        if ($statusCode == 404 && empty($correlationHdr))
            ...
            if (!empty($jsonResponse)) {
                ...
                if (isset($jsonResponse->message))
                    ...
                    $apiError = new ApiException($response->getReasonPhrase().", inspect Drupal\acf_kibo\Mozu\Api\ApiException->items property for more details");
                //$apiError = new ApiException($jsonResponse->message, $statusCode);
